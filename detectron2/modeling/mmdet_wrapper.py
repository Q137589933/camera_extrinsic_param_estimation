...
class MMDetBackbone(Backbone):
    ...
    produce a dict of tensors. This class wraps the given image_encoder to produce
    ...
    def __init__(
        ...
        *,
        ...
        """
        Args:
            backbone: either a image_encoder module or a mmdet config dict that defines a
                image_encoder. The image_encoder takes a 4D image tensor and returns a
                sequence of tensors.
            neck: either a image_encoder module or a mmdet config dict that defines a
                neck. The neck takes outputs of image_encoder and returns a
                sequence of tensors. If None, no neck is used.
            output_shapes: shape for every output of the image_encoder (or neck, if given).
                stride and channels are often needed.
            output_names: names for every output of the image_encoder (or neck, if given).
        ...
        logger.info("Initializing mmdet image_encoder weights...")
    ...
    def forward(self, x) -> Dict[str, Tensor]:
        ...
        assert isinstance(
            outs, (list, tuple)
        ), "mmdet image_encoder should return a list/tuple of tensors!"
        if len(outs) != len(self._output_shapes):
            raise ValueError(
                "Length of output_shapes does not match outputs from the mmdet image_encoder: "
