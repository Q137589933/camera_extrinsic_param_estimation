...
from .transformer import build_tokenizer, build_lang_encoder
...
class LanguageEncoder(nn.Module):
    ...
    def __init__(
            ...
            lang_projection: torch.Tensor,
            max_token_num: int,
    ):
        """
        :param tokenizer: used to transfer string to id tokens.
         use CLIP's pretrained tokenizer for default
        :param tokenizer_type: type of tokenizer
        :param lang_encoder: A transformer module used to get text embeddings
        :param lang_projection: weights shaped in [dim_text,dim_model]
        used to project the text tokens from dim_text to dim_model
        :param max_token_num: padding all the sentence to length of max_token_num
        """
        super().__init__()
    ...
    def get_text_embeddings(self, class_names, name='default', is_eval=False, prompt=True, norm=True):
        """
        :param class_names: class name of the instance ['people','car',...]
        :param name:
        :param is_eval:
        :param prompt: use prompt to extend the classnames to a sentence
        :param norm:
        :return:
        """
        if not is_eval:
